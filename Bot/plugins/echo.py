...
async def echo_http(bot: Client, update: Message):
    ...
    fetching_message = await bot.send_message(
        chat_id=update.chat.id,
        text=client.translation.FETCHING_FORMATS,
    ...
    if e_response and "nonnumeric port" not in e_response:
        ...
        await bot.edit_message_text(
            chat_id=update.chat.id,
            message_id=fetching_message.id,
            text=client.translation.NO_VOID_FORMAT_FOUND.format(str(error_message)),
    ...
    if t_response:
        ...
        if thumbnail_image and not client.custom_thumbnail.get(update.from_user.id):
            thumb_image_path = client.config.DOWNLOAD_LOCATION + "/" + str(update.from_user.id) + ".webp"
        ...
        await bot.edit_message_text(
            chat_id=update.chat.id,
            message_id=fetching_message.id,
            text=client.translation.FORMAT_SELECTION.format(thumbnail) + "\n" + client.translation.SET_CUSTOM_USERNAME_PASSWORD,
